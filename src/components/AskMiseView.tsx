...
export const AskMiseView: React.FC<AskMiseViewProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-3xl mx-auto flex-1 flex flex-col justify-start py-8 sm:py-12">
        {/* HERO HEADER: "Ask MISE" & Subtitle */}
        <div
          id="ask-mise-header"
          className={`text-center transition-all duration-300 ${
            hasMessages ? 'mb-6 sm:mb-8' : 'mb-8 sm:mb-10 pt-4 sm:pt-8'
          }`}
        >
          <div className="inline-flex items-center gap-2 mb-3">
            <span className="p-1.5 rounded-lg bg-gradient-to-br from-indigo-500/20 to-cyan-500/20 border border-indigo-500/30 text-cyan-400">
          ...
          <p className="mt-3 text-base sm:text-lg text-slate-300 max-w-xl mx-auto font-normal leading-relaxed">
            Ask about scenes, assets, decisions and continuity.
          </p>
        </div>

        {/* PRIMARY CHAT INPUT AREA */}
        <div id="primary-chat-input-container" className="w-full mb-6 relative">
          <form
            onSubmit={handleSubmit}
            className="relative flex items-center rounded-2xl bg-white/[0.03] border border-white/[0.12] hover:border-white/[0.2] focus-within:border-indigo-500/80 focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all duration-200 shadow-2xl backdrop-blur-xl group"
          >
            <input
              ref={inputRef}
              id="mise-query-input"
              ...
              disabled={isLoading}
              placeholder="Ask about approved assets, camera decisions, continuity, lighting..."
              className="w-full bg-transparent px-5 py-4 sm:py-4.5 text-sm sm:text-base text-white placeholder-slate-400 focus:outline-none disabled:opacity-50 pr-24 sm:pr-28 font-normal"
            />

            <div className="absolute right-2.5 flex items-center gap-1.5">
              <span className="hidden sm:flex items-center gap-0.5 text-[10px] font-mono text-slate-400 px-1.5 py-0.5 rounded border border-white/[0.08] bg-white/[0.02]">
              ...
              <button
                ...
                className="p-2 sm:px-3.5 sm:py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 hover:from-indigo-400 hover:to-cyan-400 text-white font-medium text-xs shadow-md shadow-indigo-500/20 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200 flex items-center gap-1.5 shrink-0"
              >
              ...
              {EXAMPLE_QUESTIONS.map((q) => {
                return (
                  <button
                    ...
                    className="text-xs sm:text-sm text-left px-3.5 py-2 rounded-xl bg-white/[0.02] hover:bg-white/[0.06] border border-white/[0.08] hover:border-indigo-500/40 text-slate-300 hover:text-white transition-all duration-150 disabled:opacity-40 flex items-center gap-2 group"
                  >
        ...
        <div id="answers-container" className="w-full space-y-6 pb-8">
          {messages.map((message) => {
            ...
            return (
              <div
                ...
                className="p-5 sm:p-7 rounded-2xl bg-white/[0.03] border border-white/[0.09] shadow-xl backdrop-blur-md animate-in fade-in duration-300 relative group"
              >
          ...
          {isLoading && (
            ...
            >
              ...
              <div className="rounded-2xl px-5 py-3.5 bg-[#131625]/90 border border-indigo-500/30 text-slate-200 shadow-xl">
